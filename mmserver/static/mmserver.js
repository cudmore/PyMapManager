...
$scope.$watch("imageBrightness", function(){
    ...
    for (var i=0; i<tmpNumTimepoint; i+=1) {
    	$scope.image[i].getElement().style.filter = "brightness(" + $scope.imageBrightness + "%)";    
...
//20171225 filter
//$scope.image[tp].getElement().style.filter = "brightness(200%)";
	    //
	...
	function addLayers(tp) {
		...
		// marker to show timepoint 0,1,2
		//var marker = new L.marker([100,100], { opacity: 0.01 }); //opacity may be set to zero
		//var tpText = parseInt(tp)
		//marker.bindTooltip(""+tpText, {permanent: true, className: "my-label", offset: [0, 0] });
		//marker.addTo(leafletRun[tp]);

		/*
		spanText = "<span>" + tp + "</span>"
		L.easyButton(spanText, function(){
		  //$scope.setslicebutton0(tp, +1)
		}).addTo( leafletRun[tp] );
		*/
...
//goal is to have a dropdown button to set options (tracing, annotations, max project, image controls)
//just do this myself
// see: https://stackoverflow.com/questions/16954605/how-can-i-use-bootstrap-multiselect-dropdown-in-angularjs
//too complicated
...
$scope.example2settings = {displayProp: 'id', showCheckAll: false, showUncheckAll:false, dynamicTitle: false};
*/
