...
function getSessions() {
	...
	var url = serverurl + '/v2/' + $scope.username + '/' + $scope.loadedMap + '/sessions'
	$http.get(url)
		.then(function(response) {
...
function getMapSegments() {
	// for some reason d3.json() did not work here?
	var url = serverurl + '/v2/' + $scope.username + '/' + $scope.loadedMap + '/mapsegments'
	$http.get(url)
		.then(function(response) {
...
/*
function getMapDynamics(mapsegment, session) {
	if (mapsegment == null) {
		mapsegment = ''
	}
	if (session == null) {
	...
	var url = serverurl + '/v2/' + $scope.username + '/' + $scope.loadedMap + '/getmapdynamics'
	url += '?mapsegment=' + mapsegment
	url += '&session=' + session

	console.log('getMapDynamics() url:', url)
	...
	$http.get(url)
		.then(function(response) {
		  ...
		  console.log('getMapDynamics() xydata:');
		  console.log(xydata);

			var stopTime = Date.now()
			var elapsedTime = (stopTime-startTime) / 1000
			console.log('getMapDynamics() REST took:', elapsedTime, 'seconds')
...
function userClick(pnt, data) {
	...
	//console.log('userClickMapSegment:' + $scope.userClickMapSegment)
	
	selectInPlotly(selRunRow, selRunCol)
	
	/*
	// update runSelection
	//Plotly.extendTraces('scatterPlot', {x: [], y: []}, [1]) 
	xNew = xydata.x[selRunRow]
	yNew = xydata.y[selRunRow]
	
	console.log('xNew:', xNew)
	console.log('yNew:', yNew)
	
	//Plotly.extendTraces('scatterPlot', {x: xNew, y: yNew}, [1]) 
	// [1] refers to 2nd trace, e.g. runSelection
	Plotly.restyle('scatterPlot', {
    	x: xNew,
    	y: yNew,
  	}, [1])

	//Plotly.redraw('scatterPlot');
  	updateScatterPlot(xydata)
  	*/
	...
	for (var thisTP=0; thisTP<tmpNumTimepoint; thisTP+=1) {
		...
		leafletRun[thisTP].on('drag', function (e) {
			if ($scope.linkTimepoints) {
				...
				var latDiff = thisCenter.lat - linkedCenter[0].lat
				var lngDiff = thisCenter.lng - linkedCenter[0].lng
...
/*
	...
	// tp 0
	//
	leafletRun[0] = L.map('myLeafletID0', {
    	crs: L.CRS.Simple,
		scrollWheelZoom: false,
		//fullscreenControl: true,
	    //fullscreenControlOptions: {
	...
	$scope.currSlice[0] = 0
	$scope.image[0] = L.imageOverlay('', bounds).addTo(leafletRun[0]);
	setMaxProject(0)
	leafletRun[0].fitBounds(bounds);
	leafletRun[0].on('click', onMapClick);

	addLayers(0)

	document.getElementById("myLeafletID0").addEventListener("keydown", myKeyDown);
	document.getElementById("myLeafletID0").addEventListener("keyup", myKeyUp);
	document.getElementById("myLeafletID0").addEventListener("wheel", myWheel);

    //scale bar (does not work for L.CRS.Simple)
    //L.control.scale().addTo(leafletRun[0]);

	//
	// tp 1
	//
	
	leafletRun[1] = L.map('myLeafletID1', {
		crs: L.CRS.Simple,
		scrollWheelZoom: false,
		//fullscreenControl: true,
		//fullscreenControlOptions: {
	...
	$scope.currSlice[1] = 0
	$scope.image[1] = L.imageOverlay('', bounds).addTo(leafletRun[1]);
	setMaxProject(1)
	leafletRun[1].fitBounds(bounds);
	leafletRun[1].on('click', onMapClick);

	addLayers(1)

	document.getElementById("myLeafletID1").addEventListener("keydown", myKeyDown);
	document.getElementById("myLeafletID1").addEventListener("keyup", myKeyUp);
	document.getElementById("myLeafletID1").addEventListener("wheel", myWheel);

	//
	// tp 2
	//
	var tmpTP = 2
	leafletRun[tmpTP] = L.map('myLeafletID2', {
		crs: L.CRS.Simple,
		scrollWheelZoom: false,
		//fullscreenControl: true,
		//fullscreenControlOptions: {
	...
	$scope.currSlice[tmpTP] = 0
	$scope.image[tmpTP] = L.imageOverlay('', bounds).addTo(leafletRun[tmpTP]);
	setMaxProject(tmpTP)
	leafletRun[tmpTP].fitBounds(bounds);
	leafletRun[tmpTP].on('click', onMapClick);

	addLayers(tmpTP)
    	
	document.getElementById("myLeafletID2").addEventListener("keydown", myKeyDown);
	document.getElementById("myLeafletID2").addEventListener("keyup", myKeyUp);
	document.getElementById("myLeafletID2").addEventListener("wheel", myWheel);
*/
	...
	function selectRun(seedTP, seedRunRow) {
		...
		for (var currTP = 0; currTP<tmpNumTimepoint; currTP+=1) {
			...
			if (stackidx >= 0) {
				...
				//need snap using image coordinates from seed tp/runRow
	...
	function addLayers(tp) {
		...
		// 20171225
		//layerGroup[tp].on("click", function (event) {
		//	var clickedMarker = event.layer;
		//	console.log('layerGroup clickedMarker:',clickedMarker) 
		//});
	...
	function getLeafletMapValues(tp, mapsegment) {
		...
		d3.json(url, function(data) {
		  ...
		  /*
		  xyzTracingLeaflet[tp] = data
		  xyzTracingLeaflet[tp].xstat = 'x'
		  xyzTracingLeaflet[tp].ystat = 'y'
		  xyzTracingLeaflet[tp].zstat = 'z'

		  // used to find location of missing data (on add/subtract)
		  //xyzTracingLeaflet[tp].sDist = 'sDist'
		  //xyzTracingLeaflet[tp].ID = 'ID'
		  */
	...
	$scope.userToggleMaxProject = function() {
		...
		if ($scope.showMaxProject) {
			...
			getLeafletData(tp, $scope.currSlice[tp]) // redraw data with no mask, currSlice is not used here
		} else {
			$scope.maskPoints = true
			setSlice(tp, $scope.currSlice[tp])
		}
